...
const DevUpdate = () => {
  ...
  useEffect(() => {
    ...
    let dt = new Date(localStorage.getItem('data_nascimento'));
    setDataNascimento(dt.toISOString().split('T')[0]);
    setHobby(localStorage.getItem('hobby'));
    setNivelId(localStorage.getItem('nivel_id'));
    }, []);  
  const updateAPIData = (event) => {
    event.preventDefault();
    axios
      .put(`http://127.0.0.1:3000/api/desenvolvedores/${id}`, { nome, sexo, data_nascimento: dataNascimento, hobby, nivel_id })
      .then((res) => {
        setMessage(<Message positive><p>{res.data.nome} ({res.data.id}) alterado com sucesso!</p></Message>);
        setNome('');
      })
      .catch((err) => {
